		...
		request(requestOpts, function(err, response, data) {
			if (err) {
				...
				if (data.slice(0, 5) == '<?xml') {
					xml2js.parseString(data, {ignoreAttrs: true , tagNameProcessors: [xml2js.processors.stripPrefix], explicitArray: false}, function(err, result) {
						// Throw an error if there was a problem reported
						if (err != null) {
							reject(new Error(err.Code + ": " + err.Message));
						} else {
							result.requestOpts = JSON.stringify(requestOpts);
							resolve(result);
						}
					});
				} else {
